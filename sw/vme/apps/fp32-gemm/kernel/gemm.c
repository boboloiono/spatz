    ...
    uint32_t ti_lo, uint32_t ti_hi)
{
    ...
    asm volatile(
        // Keep all column blocks in one call and preserve pointer state in
        // registers across block transitions.
        "9:\n"
        ...
        // b1p and b0p have advanced equally, so their distance is one packed
        // tile. Reuse the completed store-loop counter as a transition temp.
        "sub %[loop], %[b1p], %[b0p]\n"
        "sub %[a0p], %[a0p], %[loop]\n"
        "sub %[a1p], %[a1p], %[loop]\n"
        "add %[b0p], %[b0p], %[loop]\n"
        ...
        // Output streams are at row 16. Rewind 16 rows and advance 32 FP32
        // columns to reach row 0 of the next column block.
