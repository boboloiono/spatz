...
static inline float gemm_bias(uint32_t row, uint32_t col)
{
    const int32_t value = (int32_t)((row * 3u + col * 5u) & 15u) - 8;
...
int main(void)
{
    ...
    const uint32_t bias_bytes = M * N * sizeof(float);
    ...
    if (cid == 0) {
        ...
        Bias = (float *)snrt_l1alloc(bias_bytes);
        Cref = (float *)snrt_l1alloc(c_ref_bytes);
        Bpack = (float *)snrt_l1alloc(b_bytes);
        Apack = (float *)snrt_l1alloc(a_bytes);
        allocation_failed = C == NULL || Bias == NULL || Cref == NULL ||
                            Apack == NULL || Bpack == NULL;
        if (!allocation_failed) {
            ...
            for (uint32_t row = 0; row < M; ++row) {
                for (uint32_t col = 0; col < N; ++col) {
                    ...
                    Bias[packed_idx] = gemm_bias(row, col);
                }
            }
        }
    }
    ...
    if (cid < active_cores) {
        ...
        gemm_fp32(C, Bias, Apack, Bpack, ti_lo, ti_hi);
        wait_spatz();
    ...
    for (uint32_t idx = 0; idx < M * N; idx++) {
        ...
        const float ref = Cref[idx] + gemm_bias(row, col);
        float diff    = C[row * output_ld + col] - ref;
        float abs_err = diff      < 0.0f ? -diff      : diff;
        float den     = ref < 0.0f ? -ref : ref;
